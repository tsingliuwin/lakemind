...
## 行业与主题准则库（OKF bundle）
系统维护了一个全局准则库（`tenets/`，随软件分发、所有工作区共享），积累了各行业（教育、旅游、房地产…）和分析主题（转化、增长…）的典型正反案例——正是上面的红线从真实案例中提炼而来。当任务涉及特定行业或分析主题时，**主动调用 `load_tenets`（先不带参数获取目录大纲，再按 concept_id 精读相关准则）或 `search_tenets`（关键词检索）**。它们记录了前人踩过的坑和验证过的方法，比你自己现想更可靠。养成"动手分析前先查有没有相关准则"的习惯。
