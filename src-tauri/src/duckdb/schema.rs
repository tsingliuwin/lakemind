...
pub fn estimate_row_count(conn: &Connection, e: &ScanEntry) -> AppResult<Option<i64>> {
    match e.kind {
        ...
        SourceKind::Csv | SourceKind::Json | SourceKind::Excel | SourceKind::Table | SourceKind::View | SourceKind::Postgres | SourceKind::Mysql => count_view(conn, &e.view_name).map(Some),
    }
}
