...
/**
 * Minimal Arrow sidecar: download a MaxCompute table via TableTunnel +
 * ArrowTunnelRecordReader, re-serialize each batch as an Arrow IPC STREAM to
 * stdout (Rust reads it). All logs go to stderr.
 *
 *   usage: ArrowSidecar <project.table>
 *   env  : ODPS_ENDPOINT, ODPS_ACCESS_KEY_ID, ODPS_ACCESS_KEY_SECRET,
 *          [ODPS_TUNNEL_ENDPOINT], [ODPS_REGION]
 *
 * The tunnel download has NO instance-tunnel 10000-row cap; this is the bulk
...
public class ArrowSidecar {
    public static void main(String[] args) throws Exception {
        ...
        if (endpoint == null || akId == null || akSecret == null || args.length < 1) {
            System.err.println("usage: ArrowSidecar <project.table>  (env: ODPS_ENDPOINT, ODPS_ACCESS_KEY_ID/SECRET, [ODPS_TUNNEL_ENDPOINT], [ODPS_REGION])");
            System.exit(2);
        }
        String full = args[0];
